            ...
            <table class="table table-striped table-condensed receipt">
				<thead>
					<tr>
						...
						<th><?= lang("ចំនួន"); ?></th>
						<th><?= lang("តំលៃរាយ"); ?></th>
						<th><?= lang('បញ្ចុះតំលៃ');?></th>
						
						<th style="padding-left:10px;padding-right:10px;"><?= lang("តំលៃសរុប"); ?> </th>
					</tr>
				</thead>
					<tbody>
                        <?php
                        ...
                        foreach ($rows as $row) {
							...
							echo '<tr><td>#' . $r .'</td>
							<td>'.product_name($row->product_name, $printer->char_per_line) .'</td>
							<td>'.$this->sma->formatQuantity($row->quantity) .'</td>
							<td>'.($row->variant ? ' (' . $row->variant . ')' : '') . $this->sma->formatMoney($row->unit_price + $row->item_discount) .'</td><td>'.$row->discount .'</td>
							<td>'.$this->sma->formatMoney($row->subtotal); 
							'</tr>'. 
							
                            //echo '<td class="no-border border-bottom">' . $this->sma->formatQuantity($row->quantity) . ' x '.$this->sma->formatMoney($row->net_unit_price + ($row->item_tax / $row->quantity)).'</td><td class="no-border border-bottom text-right">' . $this->sma->formatMoney($row->subtotal) . '</td>';
                        ...
                        if ($return_rows) {
                            ...
                            foreach ($return_rows as $row) {
                                ...
                                echo '<tr><td class="no-border border-bottom">' . $this->sma->formatQuantity($row->quantity) . ' x '.$this->sma->formatMoney($row->net_unit_price + ($row->item_tax / $row->quantity)).'</td><td class="no-border border-bottom text-right">' . $this->sma->formatMoney($row->subtotal) . '</td><td>dis</td></tr>';
                    ...
                    <tfoot>
                        <tr class="text-right">
                            <th colspan="3"></th><th></th><th><?=lang("total");?></th>
                            <th class="text-left"><?=$this->sma->formatMoney($return_sale ? (($inv->total + $inv->product_tax)+($return_sale->total + $return_sale->product_tax)) : ($inv->total + $inv->product_tax));?></th>
                        </tr>
                        <?php
                        if ($inv->order_tax != 0) {
                        ...
                        if ($pos_settings->rounding || $inv->rounding > 0) {
                            ?>
                            <tr>
                                <th colspan="3"></th><th><th><?=lang("rounding");?></th>
                                <th class="text-right"><?= $this->sma->formatMoney($inv->rounding);?></th>
                            </tr>
                            <tr>
                                <th colspan="3"></th><th><?=lang("grand_total");?></th>
                            ...
                            ?>
                            <tr>
                                <th colspan="2"></th><th><th></th><th><?=lang("grand_total");?></th>
                                <th class="text-left"><?=$this->sma->formatMoney($return_sale ? ($inv->grand_total+$return_sale->grand_total) : $inv->grand_total);?></th>
                            </tr>
                            <?php 
                        } 
                ...
                if ($payments) {
                    ...
                    foreach ($payments as $payment) {
                        ...
                        if (($payment->paid_by == 'cash' || $payment->paid_by == 'deposit') && $payment->pos_paid) {
                            echo '<td>' . lang("paid_by1") . ': ' . lang($payment->paid_by) . '</td>';
